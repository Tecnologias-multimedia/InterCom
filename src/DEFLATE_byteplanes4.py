...
class DEFLATE_BytePlanes4(DEFLATE_raw.DEFLATE_Raw):
    ...
    def pack_stereo(self, chunk_number, chunk):
        ...
        compressed_MSB2 = zlib.compress(MSB2, level=zlib.Z_BEST_COMPRESSION)
    ...
    def pack_mono(self, chunk_number, chunk):
        channel_0_MSB2 = (chunk.astype(np.int32) // (1<<24)).astype(np.int8)
        channel_0_MSB1 = (chunk.astype(np.int32) // (1<<16)).astype(np.uint8) # Get LSB
    ...
    def unpack_stereo(self, packed_chunk):
        ...
        (chunk_number, len_compressed_MSB2, len_compressed_MSB1, len_compressed_MSB0) = struct.unpack("!HHHH", packed_chunk[:offset])
    ...
    def unpack_mono(self, packed_chunk):
        ...
        channel_MSB0 = np.frombuffer(buffer_MSB0, dtype=np.uint8)
        channel_LSB  = np.frombuffer(buffer_LSB, dtype=np.uint8)
        chunk = channel_MSB2[:len(channel_MSB2)].astype(np.uint32)*(1<<24) + channel_MSB1[:len(channel_MSB1)].astype(np.uint32)*(1<<16) + channel_MSB0[:len(channel_MSB0)].astype(np.uint16)*(1<<8) + channel_LSB[:len(channel_LSB)]
        return chunk_number, chunk
