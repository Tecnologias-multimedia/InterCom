...
# ----------------------------
# 6. Precompute CDFs for residuals
# ----------------------------
...
for b, lam in enumerate(band_lambdas):
    cdf = [0]
    s = 0
    for k in range(0, max_mag + 2):  # residuals can be 0
        p = math.exp(-lam*k)
        s += int(p * 1e6)
        cdf.append(s)
    cdfs[b] = cdf
...
bitstream = encode_tree_predict(q_coeffs, band_ids, roots, children, cdfs, alpha=0.5)
print("Encoded bitstream length:", len(bitstream))

# ----------------------------
# 8. Decoder with prediction
# ----------------------------
def decode_tree_predict(bitstream, band_ids, roots, children, cdfs, alpha=0.5):
...
decoded_q_coeffs = decode_tree_predict(bitstream, band_ids, roots, children, cdfs, alpha=0.5)
