...
def encode(plane):
    ...
    for i in range(0, length):
        fin += 64
        subplane = plane[inicio:fin]
        buffer[i] = int(np.array2string(subplane, separator = '')[1:-1],2)
        inicio = fin
    return buffer
...
def decode(plane):
    ...
    buffer = []
    for i in range(0, length):
        buffer.append(np.fromstring(np.binary_repr(plane[i], width=64),dtype=('S1')).astype(int))
