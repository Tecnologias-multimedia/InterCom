...
def audio_callback(indata, frames, time, status):
    ...
    for c in range(len(args.channels)):
        ...
        wp = pywt.WaveletPacket(data=extended_chunk[:, c], wavelet=wavelet, maxlevel=args.levels, mode="per")
        nodes = wp.get_level(args.levels, "freq")
        col_data = []
        for i, node in enumerate(nodes):
            offset = overlaped_area_size // (2**args.levels)
            sliced = node.data[offset:-offset]
            col_data.append(sliced)
        coeffs[c] = np.concatenate(col_data)
